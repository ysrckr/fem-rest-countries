...
export const Select = <T,>({ elements, ...props }: SelectProps<T>) => {
  ...
  return (
    <div className="mr-60 mb-[4rem] h-0 dark:text-white" ref={ref}>
      <button className="z-50 cursor-pointer" onClick={() => setIsOpen(!isOpen)}>
        <div className="relative">
          {" "}
          <div className={cn("absolute top-0 right-0 left-0 w-60 p-0", props.className)}>
            <HiChevronDown
              className={cn(
            ...
            <div className="absolute top-0 right-0 left-0 m-0 mx-auto h-10 w-60 rounded-md shadow-md dark:bg-dark-element">
              <p className="flex items-center px-2 pt-2">
                {props?.value ? (
